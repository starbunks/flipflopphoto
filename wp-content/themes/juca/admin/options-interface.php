...
function optionsframework_fields() {
	...
	foreach ( $options as $value ) {
		...
		case "radio":
			...
			$data_rel = (isset($value['data-rel'])) ? $value['data-rel'] : '';
			foreach ($value['options'] as $key => $option) {
				$id = $option_name . '-' . $value['id'] .'-'. $key;
				$output .= '<input class="of-input of-radio" type="radio" name="' . esc_attr( $name ) . '" id="' . esc_attr( $id ) . '" value="'. esc_attr( $key ) . '" data-rel="' . $data_rel . '" '. checked( $val, $key, false) .' /><label for="' . esc_attr( $id ) . '">' . esc_html( $option ) . '</label>';
			}
			break;
